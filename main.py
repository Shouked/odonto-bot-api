"""
OdontoBot AI – main.py – v14.1.0
────────────────────────────────────────────────────────────────────────────
• 14.0.9   fluxo reforçado (Nome → Procedimento → Data/Hora); parse “amanhã”
• 14.1.0   corrige decorators e blocos try/with em linha única (SyntaxError)
"""
...
def consultar_precos_procedimentos(
    db: Session, termo_busca: str | None = None, termo: str | None = None
) -> str:
...
# ───────────── 8. FASTAPI ─────────────
app = FastAPI(title="OdontoBot AI", version="14.1.0")
...
async def whatsapp_webhook(request: Request, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "Fluxo: 1) coletar nome completo, 2) procedimento, 3) data/hora. "
        "Evite frases repetitivas de 'finalizar cadastro' e converta 'amanhã' em datas absolutas."
    )
