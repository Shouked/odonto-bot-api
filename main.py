...
# ... (Seções 1, 2, e 3 permanecem as mesmas) ...
...
class Paciente(Base): __tablename__ = "pacientes"; id, nome_completo, telefone, endereco, email, data_nascimento = Column(Integer, primary_key=True), Column(String), Column(String, unique=True, nullable=False), Column(String), Column(String), Column(Date); agendamentos = relationship("Agendamento", back_populates="paciente", cascade="all, delete-orphan"); historico = relationship("HistoricoConversa", back_populates="paciente", cascade="all, delete-orphan")
...
# ... (Funções de ferramentas sem alteração) ...
...
# ATUALIZADO: Lista de ferramentas com descrições mais claras
...
tools = [{"type": "function", "function": {"name": "obter_proxima_etapa", "description": "Verifica se o cadastro do paciente está completo para determinar a próxima ação.", "parameters": {"type": "object", "properties": {}}}},
         ...
         {"type": "function", "function": {"name": "agendar_consulta", "description": "Ação FINAL para agendar uma consulta, usada apenas após o usuário confirmar um resumo.", "parameters": {"type": "object", "properties": {"data_hora_agendamento": {"type": "string"}, "procedimento": {"type": "string"}}, "required": ["data_hora_agendamento", "procedimento"]}}},
         ...
         {"type": "function", "function": {"name": "consultar_e_cancelar_inteligente", "description": "Cancela uma consulta. Pode receber o nome do procedimento como dica.", "parameters": {"type": "object", "properties": {"dica": {"type": "string", "description": "O procedimento ou hora que o usuário quer cancelar, se ele especificar."}}}}},
         ...
         {"type": "function", "function": {"name": "consultar_precos_procedimentos", "description": "DEVE ser usada quando o usuário perguntar 'quanto custa', 'valor', 'preço' de qualquer procedimento. A busca é flexível.", "parameters": {"type": "object", "properties": {"termo_busca": {"type": "string", "description": "O procedimento para saber o preço, ex: 'clareamento' ou 'panorâmica'."}}, "required": ["termo_busca"]}}}]

# ───────────────── 5. APP FASTAPI ───────────────────────────── #
app = FastAPI(title="OdontoBot AI", description="Automação de WhatsApp para DI DONATO ODONTO.", version="9.0.1-final")
...
async def whatsapp_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    # [BLINDADO] O "Manual de Operações" final da IA.
    system_prompt = (
        f"**Persona:** Você é Sofia, o assistente virtual profissional da clínica {NOME_CLINICA}, onde a especialista responsável é a {PROFISSIONAL}. "
        f"Seja sempre educado, prestativo e converse de forma natural. Hoje é {datetime.now().strftime('%d/%m/%Y')}.\n\n"
        "**Regras Inquebráveis:**\n"
        "- Para perguntas sobre 'o que vocês fazem' ou 'quais serviços têm', você DEVE OBRIGATORIAMENTE usar a ferramenta `listar_todos_os_procedimentos`.\n"
        "- Para QUALQUER pergunta sobre 'preço', 'valor' ou 'quanto custa', você DEVE OBRIGATORIAMENTE usar a ferramenta `consultar_precos_procedimentos`.\n"
        "- NUNCA forneça conselhos médicos. Se solicitado, recuse educadamente e diga que apenas a Dra. Valéria pode fornecer essa orientação na consulta.\n\n"
        "**Fluxo de Trabalho para Agendamentos:**\n"
        "1. Se o usuário expressar interesse em agendar, cancelar ou reagendar, seu PRIMEIRO passo é usar a ferramenta `obter_proxima_etapa` para verificar o cadastro.\n"
        "2. Siga as instruções da ferramenta para coletar os dados faltantes, UM DE CADA VEZ.\n"
        "3. Quando o cadastro estiver completo, a ferramenta te instruirá a prosseguir. Só então pergunte sobre o procedimento e use `consultar_horarios_disponiveis`.\n"
        "4. Antes de marcar, sempre apresente um resumo claro para confirmação: 'Posso confirmar seu agendamento de [Procedimento] para [Data] às [Hora] com a {PROFISSIONAL}?'\n"
        "5. Apenas após a confirmação explícita do usuário, use a ferramenta `agendar_consulta`."
    )
