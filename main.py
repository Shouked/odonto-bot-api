...
engine, SessionLocal = create_engine(DATABASE_URL, pool_pre_ping=True), sessionmaker(bind=engine, autocommit=False, autoflush=False)
...
def consultar_e_reagendar_inteligente(db: Session, telefone_paciente: str, novo_data_hora_agendamento: str) -> str:
    """Busca os agendamentos do paciente e, se houver apenas um, o reagenda. Se houver vários, pede para especificar."""
    pac = buscar_ou_criar_paciente(db, telefone_paciente)
    ...
    if len(agendamentos_ativos) > 1:
        # Se houver múltiplos, retorna a lista para o usuário escolher
    ...
    # Se houver exatamente um agendamento
...
available_functions: Dict[str, Any] = {
    ...
    "consultar_e_reagendar_inteligente": consultar_e_reagendar_inteligente, # NOVA FUNÇÃO
}
...
tools = [
    ...
    # NOVA DEFINIÇÃO DE FERRAMENTA SIMPLIFICADA
    {
        "type": "function",
        "function": {
            "name": "consultar_e_reagendar_inteligente",
            "description": "A principal ferramenta para reagendar uma consulta. Ela automaticamente encontra o agendamento do paciente e o altera para a nova data/hora fornecida. Use-a sempre que um usuário pedir para mudar, alterar ou reagendar uma consulta.",
            "parameters": {
                "type": "object",
                "properties": {"novo_data_hora_agendamento": {"type": "string", "description": "A nova data e hora no formato AAAA-MM-DD HH:MM."}},
                "required": ["novo_data_hora_agendamento"],
            },
        },
    },
]
...
# ───────────────── 4. APP FASTAPI ───────────────────────────── #
app = FastAPI(title="OdontoBot AI", description="Automação de WhatsApp para clínica odontológica.", version="2.2.0")
...
class ZapiWebhookPayload(BaseModel): phone: str; text: Optional[ZapiText] = None
...
async def whatsapp_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if not mensagem_usuario: return {"status": "ignorado", "motivo": "sem mensagem"}
    ...
    mensagens_para_ia: List[Dict[str, str]] = [{"role": "system", "content": f"Você é um atendente da 'Clínica Di Donato Odonto'. Hoje é {datetime.now().strftime('%d/%m/%Y')}. Use o histórico da conversa para ter contexto. Use as ferramentas disponíveis para realizar as ações solicitadas pelo usuário."}]
