...
tools = [
    {"type": "function", "function": {"name": "agendar_consulta", "description": "Agenda uma nova consulta para o paciente.", "parameters": {"type": "object", "properties": {"data_hora_agendamento": {"type": "string"}, "procedimento": {"type": "string"}}, "required": ["data_hora_agendamento", "procedimento"]}}},
    {"type": "function", "function": {"name": "consultar_meus_agendamentos", "description": "Lista todos os agendamentos futuros do paciente, incluindo seus IDs. Use esta função para obter os IDs necessários para reagendar ou cancelar.", "parameters": {"type": "object", "properties": {}}}},
    ...
    {"type": "function", "function": {"name": "reagendar_consulta", "description": "Altera a data ou hora de um agendamento específico usando seu ID. Importante: Se você não sabe o ID do agendamento, use a ferramenta 'consultar_meus_agendamentos' primeiro para encontrá-lo.", "parameters": {"type": "object", "properties": {"id_agendamento": {"type": "integer"}, "novo_data_hora_agendamento": {"type": "string", "description": "A nova data e hora no formato AAAA-MM-DD HH:MM."}}, "required": ["id_agendamento", "novo_data_hora_agendamento"]}}}
]

# ───────────────── 4. APP FASTAPI ───────────────────────────── #
app = FastAPI(title="OdontoBot AI", description="Automação de WhatsApp para clínica odontológica.", version="2.1.0-verbose")
...
async def whatsapp_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    historico_recente = db.query(HistoricoConversa).filter(
        ...
        HistoricoConversa.timestamp >= limite_tempo
    ).order_by(HistoricoConversa.timestamp).all()
    
    mensagens_para_ia: List[Dict[str, str]] = [
        {"role": "system", "content": f"Você é um atendente da 'Clínica Odonto Feliz'. Hoje é {datetime.now().strftime('%d/%m/%Y')}. Seja proativo: se uma ação como reagendar ou cancelar precisa de um ID que o usuário não forneceu, use a ferramenta 'consultar_meus_agendamentos' primeiro para obter a lista de agendamentos e seus IDs. Se houver mais de um, peça para o usuário especificar qual deles deseja alterar."},
    ]
