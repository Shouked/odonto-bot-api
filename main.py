...
def obter_proxima_etapa(db: Session, telefone_paciente: str) -> str:
    ...
    if not paciente.nome_completo:
        return "Ação: Paciente novo ou sem nome. Apresente-se cordialmente, diga que precisa de alguns dados para o cadastro e peça o NOME COMPLETO."
    if not paciente.data_nascimento:
        return f"Ação: Agradeça pelo nome ({paciente.nome_completo}) e peça a DATA DE NASCIMENTO no formato AAAA-MM-DD."
    if not paciente.email:
        return "Ação: Agradeça pela data de nascimento e peça o E-MAIL do paciente."
    if not paciente.endereco:
        return "Ação: Agradeça pelo email e peça o ENDEREÇO COMPLETO do paciente."
...
def atualizar_dados_paciente(db: Session, telefone_paciente: str, nome_completo: Optional[str] = None, endereco: Optional[str] = None, email: Optional[str] = None, data_nascimento: Optional[str] = None) -> str:
    paciente = buscar_ou_criar_paciente(db, tel=telefone_paciente)
    campos_atualizados = []
    if nome_completo and not paciente.nome_completo:
        paciente.nome_completo = nome_completo
        campos_atualizados.append("nome")
    if endereco and not paciente.endereco:
        paciente.endereco = endereco
        campos_atualizados.append("endereço")
    ...
    if data_nascimento and not paciente.data_nascimento:
        try:
            ...
            campos_atualizados.append("data de nascimento")
        except ValueError:
            return "Formato de data de nascimento inválido. Peça novamente no formato AAAA-MM-DD."
    db.commit()
...
tools = [{"type": "function", "function": {"name": "obter_proxima_etapa", "description": "Verifica se o cadastro do paciente está completo e determina qual a próxima ação: pedir um dado faltante ou prosseguir.", "parameters": {"type": "object", "properties": {}}}},
...
async def whatsapp_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    # [ATUALIZADO] O "Manual de Operações" final da IA.
    system_prompt = (
        f"**Persona:** Você é Sofia, o assistente virtual da clínica {NOME_CLINICA}, onde a especialista responsável é a {PROFISSIONAL}. "
        ...
        "5. Para AGENDAR, sempre use `consultar_horarios_disponiveis` antes, e depois apresente um resumo para confirmação antes de usar `agendar_consulta`."
    )
